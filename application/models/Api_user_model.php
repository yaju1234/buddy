...
class Api_user_model extends CI_Model
{
	...
	public function getUser($userid){
 		$rows = array();
     	$rows= $this->db->select('id,first_name, last_name, email,phone, IFNULL(profile_image, "", CONCAT("uploadImage/client_profile_image",profile_image)) as profile_image, IFNULL(license_image, "", CONCAT("uploadImage/client_license_image",license_image)) as license_image, is_phone_verified, is_email_verified,is_active,admin_message, status,country, state, city')->where("id",$userid)->get('traffic_users')->row_array();
     	return $rows;
	}
