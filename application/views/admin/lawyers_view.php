							...
							<tbody>
								<?php 
                                    ...
                                    foreach($client_list as $key => $list) {
										...
										if( $list['profile_image'] != '' && (strpos($list['profile_image'], 'http://') !== false || strpos($list['profile_image'], 'https://') !== false) ){
											...
											$prfImg = base_url()."uploadImage/lawyer_profile_image/".$list['profile_image'];
										}
										$i ++;
                                ?>
                                <tr class="" id="request_tr_<?=$list['id']?>">
                                    ...
                                    <td class="center">
										...
										<p>
											<?php if(isset($list['phone']) && $list['phone'] != ''){ ?>
												<?=$list['is_phone_verified'] == '1' ? '<i class="fa fa-check btn-primary" aria-hidden="true" title="Verified"></i>' : '<i class="fa fa-close btn-danger" aria-hidden="true" title="Not verified"></i>'?>
											<?php } ?>
                                    ...
                                    <td class="center">
										<?php
											foreach($list['degree'] as $k=>$deg){
												if ($deg['image'] != '') {
													$degImg = base_url()."uploadImage/degree/".$deg['image'];
										?>
													<div class="avatar">
														<a class="example-image-link" href="<?=$degImg?>" data-fancybox="lawyer-degree-<?=$deg['id']?>">
															<img src="<?=$degImg?>">
														</a>
													</div>
										<?php
                                    ...
                                    <td class="center"> <?=$list['is_active']==='1'?'Verified':'Not Verified'?> </td>
                                    ...
                                    <td class="center" style="width: 100px !important;">
										<div class="button-list1">
											<a class="btn btn-info" href="<?=base_url()?>admin/lawyers/details/<?=$list['id']?>">
